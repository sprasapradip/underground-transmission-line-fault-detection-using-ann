...
LL_cases = [10, 5]; % Line lengths for the two cases
...
for LL = LL_cases
    for i = 1:length(faults)
        ...
        % Check if yout is in the expected format (structure with time)
        if ~isstruct(yout) || ~isfield(yout, 'signals')
            error('yout is not in the expected "Structure with time" format.');
        end
...
% Load nftool with the prepared data
nftool;
